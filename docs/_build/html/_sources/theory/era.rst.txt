With the discrete LTI model (Equation
`[eq:statespace_era] <#eq:statespace_era>`__), a unit impulse input with
zero initial conditions (Equation
`[eq:state_evol_impulse] <#eq:state_evol_impulse>`__) produces an output
of constants (:math:`\mathbf{D,CB,CAB,...,CA}^{k-1}\mathbf{B}`). These
constants are called *Markov parameters* because they must be unique for
a given system – there is only one possible output for a unit impulse
input.

.. math::

   \label{eq:statespace_era}
       \begin{aligned}
...
parameters, the data can then be stacked into the generalized Hankel
matrix (Appendix `[sec:hankel] <#sec:hankel>`__) :math:`\mathbf{H}` in
Equation `[eq:hankel] <#eq:hankel>`__\  [1]_:
...
:math:`\mathbf{H}`), is shown in Equation
(`[eq:hankel_shifted] <#eq:hankel_shifted>`__):

.. math::

   \label{eq:hankel_shifted}
       \mathbf{H'}
       =
       \begin{bmatrix}
           \mathbf{y}_{2}       & \mathbf{y}_{3}        & \cdots   & \mathbf{y}_{m_{c}+1}        \\
...
By taking the dominant terms of the SVD of :math:`\mathbf{H}`, and
...
model, Equation
`[eq:statespace_reduced_era] <#eq:statespace_reduced_era>`__, is
...
.. math::
   ...
   \label{eq:statespace_reduced_era}
       \begin{aligned}
...
.. [1]
   The matrix is a *generalized* Hankel matrix because it is not
   necessarily square, and it has constant skew-diagonals in a
   block-by-block sense rather than in an element-by-element sense.
